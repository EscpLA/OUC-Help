        ...
        <scroll-view class="right" scroll-y :style="{height: wh+'px'}">
          ...
          <view class="" v-if="goodsList.length != 0">
            <navigator class="goods-container" v-for="(item, i) in goodsList"
              :url="'/subpkg/goods_detail/goods_detail?detail_type=2&item_id=' + item.id" :key="i">
              <!-- 帖子的设备分类 -->
  ...
  export default {
    data() {
      return {
        ...
        // 屏幕可用高度 = 屏幕高度 - tabbar高度 - navigationbar 高度 - 搜索栏高度（43px）。
        wh: 0,
    ...
    computed: {
      ...
      itemTime: function() {
        ...
        return index => {
          ...
          // return tools.utcToBeijing(this.goodsList[index].time).split(' ')[0];
        };
      },
    },
    methods: {
      ...
      // 获取左侧种类导航的数据
      async getCatNav() {
        console.log('qingqiu 22222');
        const {
          data: res
        } = await uni.$http.post('/Menus/')
        // console.log(res);
        // this.leftNav = res.kind;
        let tempLeftNav = res.kind;
        // 更新全局变量 LeftNav
        this.updateLeftNav(tempLeftNav)
      },
      ...
      goToSearch() {
        uni.navigateTo({
          url: '/subpkg/search/search?type=2'
        })
      },
      // 点击改变激活项
    ...
    onLoad(options) {
      ...
      info.boundingClientRect(function(data) {
        ...
        console.log('that.leftNavWidth: ', that.leftNavWidth)
      ...
      if (options != null && options.isShared == 1) {
        uni.navigateTo({
